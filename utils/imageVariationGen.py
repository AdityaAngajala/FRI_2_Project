...
def makeFolderCopies(new_folder_names, src):
...
def generatedFilteredImages(new_folder_names, src):
    srcImages = src
    # loop through all the files in the original photos folder
    for file in os.listdir(srcImages):
        ...
        filters = {
            "Brighter": (True, change_Bright(img, 10), new_folder_names[0]),
            "Darker": (True, change_Bright(img, -10), new_folder_names[1]),
            "Warmer": (False, convert_temp(im, 7000), new_folder_names[2]),
            "Cooler": (False, convert_temp(im, 9000), new_folder_names[3]),
        }

        # save the filtered images to the output folders
        for name, filter in filters.items():
            # get the right output folder
            output_path = filter[2]
...
def generateData(src):
    ...
    makeFolderCopies(new_folder_names, src)
    generatedFilteredImages(new_folder_names, src)
